...
try {
    ...
    $stmt = db()->prepare("SELECT file_path FROM whatsapp_event_files WHERE id = ?");
    $stmt->execute([$fileId]);
    $file = $stmt->fetch();
    ...
    $stmtDelete = db()->prepare("DELETE FROM whatsapp_event_files WHERE id = ?");
    $stmtDelete->execute([$fileId]);
    
    echo json_encode(['success' => true]);
    
} catch (Exception $e) {
    error_log("[WHATSAPP_EVENT_FILE_DELETE] Erro: " . $e->getMessage());
    echo json_encode([
        'success' => false,
        'error' => $e->getMessage()
    ]);
