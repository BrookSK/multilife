...
// JavaScript: filtragem client-side por nome, telefone e especialidade + seleção em massa
echo '<script>';
echo '(function(){';
...
echo '  var itens = lista.querySelectorAll(".participante-item");';
...
echo '  function normalizar(s){ return (s||"").toString().toLowerCase().normalize("NFD").replace(/[\\u0300-\\u036f]/g,""); }';
echo '  function soDigitos(s){ return (s||"").toString().replace(/\\D/g,""); }';
...
echo '    var visiveis = 0;';
echo '    itens.forEach(function(item){';
...
echo '  }';
echo '  if(fNome) fNome.addEventListener("input", aplicarFiltro);';
echo '  if(fTel) fTel.addEventListener("input", aplicarFiltro);';
echo '  if(fEsp) fEsp.addEventListener("change", aplicarFiltro);';
echo '  if(fCidade) fCidade.addEventListener("change", aplicarFiltro);';
echo '  lista.addEventListener("change", function(e){ if(e.target && e.target.classList.contains("participante-check")) atualizarSelecionados(); });';
echo '  var btnVis = document.getElementById("btnSelecionarVisiveis");';
echo '  if(btnVis) btnVis.addEventListener("click", function(){';
echo '    itens.forEach(function(item){ if(item.style.display !== "none"){ var cb = item.querySelector(".participante-check"); if(cb) cb.checked = true; } });';
echo '    atualizarSelecionados();';
echo '  });';
echo '  var btnLimpar = document.getElementById("btnLimparSelecao");';
echo '  if(btnLimpar) btnLimpar.addEventListener("click", function(){';
echo '    lista.querySelectorAll(".participante-check").forEach(function(cb){ cb.checked = false; });';
echo '    atualizarSelecionados();';
echo '  });';
echo '  aplicarFiltro();';
echo '})();';
