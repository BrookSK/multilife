...
$sql = 'SELECT t.id, t.name, t.event_type, t.health_insurer_id, t.is_active, t.created_at,
...
$sql .= ' ORDER BY t.event_type, h.name, t.name';
...
view_header('Templates de E-mail');
...
echo '<div>';
echo '<div style="font-size:22px;font-weight:900">Templates de E-mail HTML</div>';
echo '<div style="margin-top:6px;color:hsl(var(--muted-foreground));font-size:14px;line-height:1.5">Gerenciar templates personalizados para envio de propostas e comunicações</div>';
echo '</div>';
echo '<div style="display:flex;gap:10px;flex-wrap:wrap">';
echo '<a class="btn btnPrimary" href="/admin_email_templates_edit.php">+ Novo Template</a>';
echo '<a class="btn" href="/admin_settings.php">Voltar</a>';
echo '</div>';
echo '</div>';

echo '<form method="get" action="/admin_email_templates.php" style="margin-top:14px;display:flex;gap:10px;flex-wrap:wrap">';
...
echo '<a class="btn" href="/admin_email_templates.php">Limpar</a>';
echo '</form>';
...
echo '<table>';
echo '<thead><tr>';
echo '<th>ID</th><th>Nome</th><th>Evento</th><th>Operadora</th><th>Status</th><th style="text-align:right">Ações</th>';
echo '</tr></thead><tbody>';

if (empty($templates)) {
    echo '<tr><td colspan="6" style="text-align:center;padding:40px;color:hsl(var(--muted-foreground))">Nenhum template encontrado</td></tr>';
} else {
    foreach ($templates as $t) {
        $eventLabel = $events[$t['event_type']] ?? h((string)$t['event_type']);
        ...
        echo '<a class="btn btnSmall" href="/admin_email_templates_edit.php?id=' . (int)$t['id'] . '">Editar</a> ';
        echo '<form method="post" action="/admin_email_templates_delete_post.php" style="display:inline" onsubmit="return confirm(\'Confirma exclusão?\');">';
