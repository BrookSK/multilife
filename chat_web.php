...
echo '  addDebugLog("❌ ERRO JAVASCRIPT DETECTADO", "error");';
echo '  addDebugLog("========================================", "error");';
echo '  addDebugLog("Mensagem: " + msg, "error");';
echo '  addDebugLog("Arquivo: " + url, "error");';
echo '  addDebugLog("Linha: " + line + " | Coluna: " + col, "error");';
echo '  addDebugLog("", "error");';
echo '  ';
echo '  // Diagnóstico do tipo de erro';
echo '  if(msg.includes("Unexpected end of input")) {';
echo '    addDebugLog("DIAGNÓSTICO:", "warn");';
echo '    addDebugLog("Tipo: Bloco JavaScript não fechado", "warn");';
echo '    addDebugLog("ESPERADO: Todos os blocos { } devem estar fechados", "warn");';
echo '    addDebugLog("RECEBIDO: Fim do arquivo antes de fechar todos os blocos", "warn");';
echo '    addDebugLog("CAUSA COMUM: if, function, for sem fechar com }", "warn");';
echo '  } else if(msg.includes("is not defined")) {';
echo '    const funcName = msg.split(" ")[0];';
echo '    addDebugLog("DIAGNÓSTICO:", "warn");';
echo '    addDebugLog("Tipo: Função não definida", "warn");';
echo '    addDebugLog("ESPERADO: Função " + funcName + " deve ser definida antes de ser chamada", "warn");';
echo '    addDebugLog("RECEBIDO: Tentativa de chamar " + funcName + " que não existe", "warn");';
echo '    addDebugLog("CAUSA COMUM: Erro de sintaxe impediu definição da função", "warn");';
echo '  }';
echo '  addDebugLog("", "error");';
echo '  ';
echo '  // Estado das variáveis importantes';
echo '  addDebugLog("ESTADO DAS VARIÁVEIS:", "warn");';
echo '  addDebugLog("- messagesContainer: " + (typeof messagesContainer !== "undefined" ? (messagesContainer ? "OK" : "null") : "undefined"), "warn");';
echo '  addDebugLog("- chatId: " + (typeof chatId !== "undefined" ? chatId : "undefined"), "warn");';
echo '  addDebugLog("- sendForm: " + (typeof sendForm !== "undefined" ? (sendForm ? "OK" : "null") : "undefined"), "warn");';
echo '  addDebugLog("- textarea: " + (typeof textarea !== "undefined" ? (textarea ? "OK" : "null") : "undefined"), "warn");';
echo '  addDebugLog("", "error");';
echo '  ';
echo '  // Funções esperadas';
echo '  addDebugLog("FUNÇÕES ESPERADAS:", "warn");';
echo '  addDebugLog("- openAssignmentModal: " + typeof openAssignmentModal, "warn");';
echo '  addDebugLog("- loadGroupsByFilter: " + typeof loadGroupsByFilter, "warn");';
echo '  addDebugLog("- sendGroupInvite: " + typeof sendGroupInvite, "warn");';
echo '  addDebugLog("- closeInfoPanel: " + typeof closeInfoPanel, "warn");';
echo '  addDebugLog("", "error");';
echo '  ';
echo '  if(error && error.stack) {';
echo '    addDebugLog("STACK TRACE:", "error");';
echo '    addDebugLog(error.stack, "error");';
echo '    addDebugLog("", "error");';
echo '  }';
echo '  ';
echo '  addDebugLog("========================================", "error");';
...
echo '      .then(r => r.text())';
echo '      .then(html => {';
echo '        const lines = html.split("\\n");';
echo '        const errorLine = line - 1;';
echo '        const start = Math.max(0, errorLine - 2);';
echo '        const end = Math.min(lines.length, errorLine + 3);';
echo '        addDebugLog("CÓDIGO AO REDOR DO ERRO:", "warn");';
echo '        for(let i = start; i < end; i++) {';
echo '          const prefix = (i === errorLine) ? ">>> " : "    ";';
echo '          const lineNum = (i + 1).toString().padStart(4, " ");';
echo '          addDebugLog(prefix + lineNum + " | " + lines[i].substring(0, 150), i === errorLine ? "error" : "warn");';
echo '        }';
echo '        addDebugLog("", "error");';
echo '        ';
echo '        // Validar estrutura de blocos';
echo '        addDebugLog("VALIDAÇÃO DE ESTRUTURA:", "warn");';
echo '        let openBraces = 0;';
echo '        let closeBraces = 0;';
echo '        let openParens = 0;';
echo '        let closeParens = 0;';
echo '        for(let i = 0; i < lines.length; i++) {';
echo '          const line = lines[i];';
echo '          openBraces += (line.match(/\\{/g) || []).length;';
echo '          closeBraces += (line.match(/\\}/g) || []).length;';
echo '          openParens += (line.match(/\\(/g) || []).length;';
echo '          closeParens += (line.match(/\\)/g) || []).length;';
echo '        }';
echo '        addDebugLog("Chaves { abertos: " + openBraces + " | fechados: " + closeBraces, openBraces === closeBraces ? "success" : "error");';
echo '        addDebugLog("Parênteses ( abertos: " + openParens + " | fechados: " + closeParens, openParens === closeParens ? "success" : "error");';
echo '        if(openBraces !== closeBraces) {';
echo '          addDebugLog("ESPERADO: " + openBraces + " chaves fechadas", "error");';
echo '          addDebugLog("RECEBIDO: " + closeBraces + " chaves fechadas", "error");';
echo '          addDebugLog("FALTAM: " + (openBraces - closeBraces) + " chaves }", "error");';
echo '        }';
echo '        if(openParens !== closeParens) {';
echo '          addDebugLog("ESPERADO: " + openParens + " parênteses fechados", "error");';
echo '          addDebugLog("RECEBIDO: " + closeParens + " parênteses fechados", "error");';
echo '          addDebugLog("FALTAM: " + (openParens - closeParens) + " parênteses )", "error");';
echo '        }';
echo '        addDebugLog("========================================", "error");';
echo '      })';
echo '      .catch(e => {';
echo '        addDebugLog("Não foi possível buscar código fonte: " + e.message, "warn");';
echo '      });';
echo '  } catch(e) {';
echo '    addDebugLog("Erro ao processar debug: " + e.message, "warn");';
echo '  }';
echo '  return false;';
echo '};';
...
echo '  ';
echo '  const chatName = "' . addslashes($chatName) . '";';
echo '  const chatId = "' . addslashes($selectedChat) . '";';
echo '  document.getElementById("professionalName").textContent = chatName || chatId;';
