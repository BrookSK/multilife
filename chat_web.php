...
echo '<p style="font-size:11px;color:#dc2626;margin:4px 0 0;font-weight:600">💸 DESPESA - Custo do atendimento</p>';
echo '</div>';
echo '<div>';
...
echo '<p style="font-size:11px;color:#10b981;margin:4px 0 0;font-weight:600">💰 RECEITA - Valor que será recebido</p>';
echo '</div>';
echo '</div>';
echo '<p style="font-size:12px;color:#667781;margin:-8px 0 16px">Valores mínimos serão validados conforme configuração do serviço. <strong>Lucro = Autorizado - Acordado</strong> (Receita - Despesa)</p>';
...
echo '.whatsapp-chat-item.active{background:#e8f5e9}';
...
echo '.whatsapp-main{flex:1;display:flex;flex-direction:column;background:#ffffff}';
echo '.whatsapp-chat-header{padding:12px 16px;background:#ffffff;border-bottom:1px solid #d1d7db;display:flex;align-items:center;justify-content:space-between}';
...
echo '.whatsapp-input-area{padding:12px 16px;background:#ffffff;border-top:1px solid #d1d7db;display:flex;gap:8px;align-items:flex-end}';
...
// Função para calcular número de sessões baseado na frequência e duração
...
function calculateTotals() {
    const sessionsInput = document.getElementById("selectProfTotalSessions");
    const agreedInput = document.getElementById("selectProfAgreedValue");
    const proposalInput = document.getElementById("selectProfProposalValue");
    
    const totalSessions = sessionsInput ? parseFloat(sessionsInput.value) || 0 : 0;
    const agreedValue = agreedInput ? parseFloat(agreedInput.value) || 0 : 0;
    const proposalValue = proposalInput ? parseFloat(proposalInput.value) || 0 : 0;
    
    const totalCost = totalSessions * agreedValue;
    const totalRevenue = totalSessions * proposalValue;
    
    console.log("Calculando totais:", {totalSessions, agreedValue, proposalValue, totalCost, totalRevenue});
    ...
    if (costElement) costElement.textContent = formatter.format(totalCost);
    if (revenueElement) revenueElement.textContent = formatter.format(totalRevenue);
}
...
// Script para envio de mensagens via AJAX (sem recarregar página)
echo '<script>';
echo '(function(){';
echo '  var form = document.getElementById("sendMessageForm");';
echo '  if(!form) return;';
echo '  ';
echo '  form.addEventListener("submit", function(e){';
echo '    e.preventDefault();';
echo '    ';
echo '    var textarea = form.querySelector("textarea[name=message]");';
echo '    var message = textarea.value.trim();';
echo '    if(!message) return;';
echo '    ';
echo '    var sendBtn = form.querySelector("button[type=submit]");';
echo '    sendBtn.disabled = true;';
echo '    sendBtn.style.opacity = "0.5";';
echo '    ';
echo '    // Adicionar mensagem ao chat imediatamente (otimista)';
echo '    var chatArea = document.querySelector(".whatsapp-messages");';
echo '    if(chatArea){';
echo '      var msgDiv = document.createElement("div");';
echo '      msgDiv.className = "whatsapp-message sent";';
echo '      msgDiv.innerHTML = \'<div class="whatsapp-bubble sent"><div class="whatsapp-text">\' + message.replace(/\\n/g,"<br>") + \'</div><div class="whatsapp-time">\' + new Date().toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit"}) + \' ✓</div></div>\';';
echo '      chatArea.appendChild(msgDiv);';
echo '      chatArea.scrollTop = chatArea.scrollHeight;';
echo '    }';
echo '    ';
echo '    // Enviar via AJAX';
echo '    var formData = new FormData(form);';
echo '    fetch(form.action, {';
echo '      method: "POST",';
...
echo '        // Atualizar o checkmark para duplo';
echo '        if(msgDiv){';
echo '          var timeEl = msgDiv.querySelector(".whatsapp-time");';
echo '          if(timeEl) timeEl.innerHTML = timeEl.innerHTML.replace("✓","✓✓");';
echo '        }';
echo '      } else {';
echo '        // Remover mensagem otimista e mostrar erro';
echo '        if(msgDiv && chatArea) chatArea.removeChild(msgDiv);';
echo '        alert("Erro ao enviar: " + (data.error || "Erro desconhecido"));';
echo '      }';
echo '    })';
echo '    .catch(function(err){';
echo '      if(msgDiv && chatArea) chatArea.removeChild(msgDiv);';
echo '      alert("Erro de conexão: " + err.message);';
echo '    })';
echo '    .finally(function(){';
echo '      sendBtn.disabled = false;';
echo '      sendBtn.style.opacity = "1";';
echo '    });';
echo '  });';
echo '  ';
echo '  // Enter para enviar (Shift+Enter para nova linha)';
echo '  var ta = form.querySelector("textarea[name=message]");';
echo '  if(ta){';
echo '    ta.addEventListener("keydown", function(e){';
echo '      if(e.key === "Enter" && !e.shiftKey){';
echo '        e.preventDefault();';
echo '        form.dispatchEvent(new Event("submit", {cancelable:true}));';
echo '      }';
echo '    });';
echo '    // Auto-resize textarea';
echo '    ta.addEventListener("input", function(){';
echo '      this.style.height = "auto";';
echo '      this.style.height = Math.min(this.scrollHeight, 120) + "px";';
echo '    });';
echo '  }';
echo '})();';
echo '</script>';
