...
$sql = 'SELECT fe.id, fe.amount, 
               ...
               COALESCE(hi_direct.name, hi.name) as operadora,
               cl.name as cliente
        ...
        LEFT JOIN health_insurers hi_direct ON hi_direct.id = fe.health_insurer_id
...
// ITEM 12: Gráficos de Contas a Pagar (SVG no servidor)
...
echo svg_bar_chart(
    $chartLabels,
    [
        ['name' => 'Pago', 'color' => '#10b981', 'data' => $chartPago],
        ['name' => 'A Pagar', 'color' => '#dc2626', 'data' => $chartPendente],
    ],
    300
);
echo '</section>';
...
echo svg_donut_chart([
    ['label' => 'Pago', 'value' => $totalPago, 'color' => '#10b981'],
    ['label' => 'A Pagar', 'value' => $totalPendente, 'color' => '#dc2626'],
], 220);
echo '</section>';
...
echo '<table>';
echo '<thead><tr>';
echo '<th>ID</th><th>Agendamento</th><th>Data</th><th>Fornecedor</th><th>Operadora / Cliente</th><th>Ligação</th><th>Centro de Custo</th><th>Valor</th><th>Status</th><th style="text-align:right">Ações</th>';
echo '</tr></thead><tbody>';
foreach ($rows as $r) {
    echo '<tr>';
    echo '<td>' . (int)$r['id'] . '</td>';
