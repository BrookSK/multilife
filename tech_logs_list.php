...
echo '<div>';
echo '<div style="font-size:22px;font-weight:800">Logs Técnicos</div>';
echo '<div style="margin-top:6px;color:rgba(234,240,255,.72);font-size:14px;line-height:1.6">Integrações (OpenAI/Evolution/ZapSign/SMTP/Webhooks).</div>';
echo '</div>';
...
foreach ($rows as $r) {
    ...
    echo '<td style="padding:12px">' . h((string)($r['http_status'] ?? '')) . '</td>';
    echo '<td style="padding:12px">' . h((string)$r['attempts']) . '</td>';
    echo '<td style="padding:12px">' . h(mb_strimwidth((string)($r['error_message'] ?? ''), 0, 80, '...')) . '</td>';
