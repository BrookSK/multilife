...
echo '<div>';
echo '<div style="font-size:22px;font-weight:900">Logs de Acesso a Prontuário</div>';
echo '<div style="margin-top:6px;color:hsl(var(--muted-foreground));font-size:14px;line-height:1.6">Registra abertura do prontuário por usuário.</div>';
echo '</div>';
...
echo '<input name="patient_id" value="' . h($patientId) . '" placeholder="Patient ID" style="width:160px">';
echo '<input name="user_id" value="' . h($userId) . '" placeholder="User ID" style="width:160px">';
...
foreach ($rows as $r) {
    ...
    echo '<td style="font-weight:700">' . h((string)$r['patient_name']) . ' (#' . (int)$r['patient_id'] . ')</td>';
    echo '<td>' . h((string)$r['user_name']) . ' (#' . (int)$r['user_id'] . ')</td>';
    echo '<td>' . h((string)$r['action']) . '</td>';
    echo '<td>' . h((string)($r['ip_address'] ?? '')) . '</td>';
    echo '</tr>';
}
if (count($rows) === 0) {
