...
$sqlReceitas = "
    SELECT 
        ...
        CONCAT('Receita - Atendimento #', pa.id, ' - ', pa.session_quantity, ' sessões - ', p.full_name) as description,
...
$sqlDespesas = "
    SELECT 
        ...
        CONCAT('Despesa - Atendimento #', pa.id, ' - ', pa.session_quantity, ' sessões - ', p.full_name) as description,
        pa.created_at as entry_date,
        pa.created_at,
        CASE WHEN pa.status = 'paid' THEN 'paid' ELSE 'pending' END as status,
        p.full_name as patient_name,
