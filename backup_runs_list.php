...
echo '<div>';
echo '<div style="font-size:22px;font-weight:900">Backups</div>';
echo '<div style="margin-top:6px;color:hsl(var(--muted-foreground));font-size:14px;line-height:1.6">Registro de execuções (stub). Não executa dump real ainda.</div>';
echo '</div>';
...
echo '<select name="kind" style="min-width:220px">';
...
foreach ($rows as $r) {
    echo '<tr>';
    echo '<td>' . (int)$r['id'] . '</td>';
    echo '<td>' . h((string)$r['kind']) . '</td>';
    echo '<td>' . h((string)$r['status']) . '</td>';
    echo '<td>' . h((string)$r['started_at']) . '</td>';
    echo '<td>' . h((string)($r['finished_at'] ?? '')) . '</td>';
    echo '<td>' . h((string)($r['output_path'] ?? '')) . '</td>';
    echo '<td>' . h((string)($r['error_message'] ?? '')) . '</td>';
    echo '</tr>';
}
if (count($rows) === 0) {
